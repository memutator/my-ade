...
- `dist/ade-<ver>.AppImage`
- `dist/ade_<ver>_amd64.deb`
...
Known gap: the hook scripts `ade-hook.cjs` / `ade-opencode-plugin.js` are only
...
Installs to `/opt/ade`, links `/usr/bin/ade` via `update-alternatives`.

`postinst`:

- probes `unshare --user` — if user namespaces are unavailable it runs
  `chmod 4755 /opt/ade/chrome-sandbox` (the SUID fallback), else `chmod 0755`
- installs the bundled AppArmor profile
  (`/opt/ade/resources/apparmor-profile` → `/etc/apparmor.d/ade`, an abi-4.0
...
Desktop entry (`/usr/share/applications/ade.desktop`):
`Exec=/opt/ade/ade %U`, `Icon=ade`, `StartupWMClass=ade`. `desktopName: "ade"`
...
Icon lands at `/usr/share/icons/hicolor/512x512/apps/ade.png` (from
`build/icon.png`).
...
1. `ADE_NODE` / `NODE_BINARY` env vars
...
Override with `ADE_NODE=/path/to/node`. When nothing resolves the app still
runs — terminals just stay dead.

## Install / upgrade

```bash
sudo dpkg -i dist/ade_*_amd64.deb
```
