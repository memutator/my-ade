...
| `npm run typecheck`      | `typecheck:node` (main + preload), `typecheck:web` (renderer), then `typecheck:packages` (every package source file)                                    |
| `npm run typecheck:packages` | verifies each package entrypoint and source file is inside its own tsconfig program, then runs `tsc --noEmit` per package                          |
| `npm run lint`           | `eslint --cache .`                                                                                                                                      |
| `npm run check:boundaries` | resolves every import and applies the package dependency policy (`tools/check-boundaries.mjs`)                                                         |
| `npm run test:boundaries` | runs that policy against a temporary fixture tree (`--self-test`)                                                                                       |
| `npm run check:docs`     | checks that repo-relative Markdown links resolve (`tools/check-docs-links.mjs`)                                                                         |
| `npm run test:domain`    | synthetic domain smokes: storage migration, catalog/inventory, Pack pipeline, collection, ledger, aggregates/statistics, auth, registry (`--include-runtime` adds the launch/access suite) |
| `npm run format`         | `prettier --write .`                                                                                                                                     |
| `npm run build:services` | bundles `mahasd`, `execution-host`, and the `mahas` CLI into `out/services/*.mjs` and boot-tests each artifact                                     |
| `npm run build`          | typecheck + boundary + docs checks + `electron-vite build` (which also runs the service bundler) → `out/`                                               |
| `npm run start`          | `electron-vite preview` of the built app                                                                                                                |
| `npm run build:unpack`   | build + `electron-builder --dir` → `dist/linux-unpacked`                                                                                                |
| `npm run build:linux`    | build + `electron-builder --linux` → `dist/`                                                                                                            |
...
There is no unit-test runner; verification is the scripted checks in
[verification.md](verification.md) plus live checks via CDP (evaluate store
state, screenshot panes, simulate flows).

```bash
npm run check:boundaries   # import direction policy over every source file
npm run test:boundaries    # the policy's own fixture self-test
npm run check:docs         # repo-relative Markdown links resolve
npm run typecheck          # node + web + packages
npm run test:domain        # synthetic domain smokes (unit + pipeline)
npm run build:services     # bundle the daemons, then boot-test the artifacts
```

The domain smokes build throwaway SQLite databases and Pack snapshots under
`/tmp`; none of them reads a real profile, credential, or session log. Their
results and the checks that are currently failing are recorded in
