...
`integrations/packs/harness-runtime/hooks/mahas-hook.cjs` is the bridge every command-style hook calls — it ships inside the `builtin.harness-runtime` Pack and main installs a copy into the mahas config dir. It reads
...
- starts at EOF — history is not replayed for the live channel; truncates
  the file past 2 MB. Durability comes from ingest, not the tail: every
  received record is forwarded to the daemon through `session.hook.ingest`
  (`src/main/agentEventIngest.ts`) and the renderer only sees the event after
  the all-record acknowledgement (records merge into ≤512-record/≤2 MiB
  chunks with overflow protection). The daemon's own hook-stream reader
  ingests the same file independently with checkpoints, so events become
  stored session history even with no UI attached
...
mahas offers to reopen the agent sessions that were **alive at last
shutdown**. On the next launch, activating a workspace with resumable
sessions pops a dialog offering to reopen them all in one click.

**Tracking**: resume is a domain read (`src/renderer/src/resume.ts`). The
daemon's session store owns sessions, handles (native id + resume support),
and attachments — fed by the durable hook ingest above. The desktop's own
persisted map (`resumeSessions` in `mahas-state.json`, records of
`{sessionId, provider, cwd, wsId, paneId, tabId}`) is legacy input that is
migrated once: each refresh hands the records the store does not know yet to
`session.desktop.import` (bounded, placement only) and keeps the returned
native → canonical mapping. Placement then comes from the store row's
metadata or that exact mapping — never from a native-id suffix match, and
never from another installation's session. The map is only a fallback while
the store cannot answer. A tab still hosts one live session: a new record
claims its tab outright, and weaker signals (`idle`/`other`) never displace
the recorded session — sub-session churn demoted to `other` would otherwise
steal records from the resumable parent.

Ended runs are not offered. A session whose last event is `session-end`, or
whose store metadata marks it ended, is skipped — `SessionEnd`, opencode
`session.deleted`, grok's teardown `Stop`, the agent process leaving the
tab's process tree, the pty exiting, or its tab/pane/workspace closing all
end a candidacy, and hydration prunes records whose pane/tab no longer
exists.
...
the children). Those deaths emit `session-end` events — which would normally
end a candidacy — so main's final synchronous save stamps each record with
`shutdown: {runId, at}` (`src/main/state/persistence.ts`). The importer keeps
a candidate only when the ending event is exactly that app-shutdown death:
the event's `mahasSession` equals `runId`, its timestamp falls within 30 s of
the checkpoint, and its `paneId`/`tabId` match the record. An earlier end, a
child or foreign session's end, or another run's end can never be revived —
the event stays historical fact. A `beforeunload` guard also keeps dying
sessions' `exit` events from stripping the set mid-persist.
...
- **`e2e.mjs`** — a CDP driver that boots the built app (`npm run build` →
  `electron .`, not electron-vite dev) under an allowlisted environment with
  an isolated `HOME`/`XDG_CONFIG_HOME`, an OS-allocated CDP port (no fixed-port
  collisions), and exact-PID cleanup of every process it birthed. It drives
  the app via `window.__mahas` (the store handle `main.tsx` exposes) +
  ...
  pty sessions preserved), `lifetime` (minimize/restore and float/detach
  keep the same xterm DOM node and the same live pty — the agent keeps
  emitting with no respawn). Harness
