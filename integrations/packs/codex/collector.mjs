#!/usr/bin/env node
// Codex local session collector (AdapterPack capability: identify, sessions, usage).
//
// Read-only. The rollout JSONL files are append-only, so the checkpoint is a
// confirmed-newline byte offset: the cursor advances only past records whose
// terminating newline was observed. Source identity (dev/inode/birth) is
// separate from the cumulative counter epoch: a rotated file is a new source
// generation and requires rediscovery, a decreasing counter opens a new epoch.
...
const MODEL_NAMESPACE = 'codex.rollout.token-usage'
const MEASUREMENT_KEY = 'codex.session.total-token-usage'
...
const namespaceFor = (configRoot, dataRoot) =>
  'codex-installation:' + sha(resolve(configRoot) + String.fromCharCode(0) + resolve(dataRoot))
...
function sourceFor(path, namespace) {
  ...
  return {
    sourceKey: 'codex:rollout:' + sha(namespace + String.fromCharCode(0) + real),
    kind: 'file',
...
/** Shared shape for outcomes where records cannot be read from this source. */
...
function readChunk(path, source, rawCursor, maxBytes, maxRecords) {
  ...
  let counterEpoch =
    compatible && string(cursor.counterEpoch) ? cursor.counterEpoch : 'source:' + source.generation
  let lastCounterTotal = compatible ? number(cursor.lastCounterTotal) : null
  const diagnostics = []
  if (offset > stat.size) {
    ...
    diagnostics.push(
      diagnostic(
        'source.truncated',
        'The JSONL file shrank; collection restarted with a new counter epoch.',
        {
          previousOffset,
  ...
  return { stat, lines, diagnostics, offset, nextOffset, counterEpoch, lastCounterTotal }
}
...
function usageValues(raw) {
  ...
  return {
    inputTotal: input,
    outputTotal: output,
    // Cache read is contained in input natively, so a derived total is only
    // known when both components were observed (an unobserved field is unknown).
    total: explicitTotal ?? (input !== null && output !== null ? input + output : null),
    cacheReadInput: number(
      raw.cached_input_tokens ?? raw.cache_read_input_tokens ?? raw.cachedReadTokens
    ),
    cacheWriteInput: number(
      raw.cache_creation_input_tokens ?? raw.cache_write_input_tokens ?? raw.cacheWriteTokens
    ),
...
function usageSemantics(values, raw) {
  ...
  return {
    ...
    nativeFields: {
      ...
      total: 'total_tokens',
      cacheReadInput: 'cached_input_tokens',
      reasoningOutput: 'reasoning_output_tokens'
    }
  }
}

function nativeSessionKey(path) {
...
function collect(request, payload) {
  ...
  const namespace = String(locator.namespace || 'codex-data:' + sha(resolve(path)))
  ...
  let native = string(cursorIn.sessionNativeKey) ?? nativeSessionKey(path)
  const metaRecord = records.find(
    ({ value }) => value.type === 'session_meta' && string(object(value.payload).id)
  )
  if (metaRecord) native = string(object(metaRecord.value.payload).id) ?? native
  const observedAt = timestamp(metaRecord?.value?.timestamp) ?? Math.trunc(chunk.stat.mtimeMs)
  const sessionRecordKey = supplied.sourceKey + ':session:' + encodeURIComponent(native)

  const usageReadings = []
  const usageAttributionHints = []
  let epoch = chunk.counterEpoch
  let previousTotal = chunk.lastCounterTotal
  if (capability === 'usage') {
    for (const record of records) {
      const item = object(record.value)
      const nativePayload = object(item.payload)
      let raw = null
      if (item.type === 'event_msg' && nativePayload.type === 'token_count') {
        raw = object(
          object(nativePayload.info).total_token_usage ?? nativePayload.total_token_usage
        )
      } else if (item.type === 'token_usage_record') {
        raw = object(nativePayload.usage)
      }
      if (!raw || !Object.keys(raw).length) continue
      const nativeRecordId = string(item.id) ?? string(nativePayload.id)
      const key = nativeRecordId
        ? 'codex:record:' + nativeRecordId
        : supplied.sourceKey + ':generation:' + current.generation + ':byte:' + record.start
      const values = usageValues(raw)
      if (previousTotal !== null && values.total !== null && values.total < previousTotal) {
        epoch = 'reset:' + sha(current.generation + String.fromCharCode(0) + key)
        chunk.diagnostics.push(
          diagnostic(
            'counter.reset',
            'The cumulative token counter decreased; a new counter epoch was opened.',
            {
              sourceRecordKey: key
            }
          )
      ...
      if (values.total !== null) previousTotal = values.total
      ...
      usageReadings.push({
        ...
        mode: 'cumulative',
        counterScope: namespace + ':' + native + ':total-token-usage',
        counterEpoch: epoch,
        values,
        semantics: usageSemantics(values, raw),
        timeCoverage:
          at === null
            ? { kind: 'unknown', reason: 'native usage record has no trustworthy timestamp' }
            : {
                kind: 'point',
                at,
                basis: 'native-record',
      ...
      const model = string(nativePayload.model) ?? string(item.model)
      if (model) {
        usageAttributionHints.push({
          ...
          confidence: 'observed',
          evidence: [
            { sourceRecordKey: key, description: 'Model name stored on the native usage record' }
          ]
        })
      }
    }
  ...
  // The ledger resolves session identity from the same batch, so a batch that
  // reports usage also carries the session and handle those readings belong to.
          ...
          evidence: [
            {
              sourceRecordKey: sessionRecordKey,
              description:
                'Rollout file carries the native session id; the CLI resumes it by that id.'
            }
          ]
        }
      ]
  ...
  return {
    ...
    nextCursor: {
      ...
      counterEpoch: epoch,
      ...(previousTotal === null ? {} : { lastCounterTotal: previousTotal })
    },
    exhausted,
    coverage: {
      completeness,
...
function identify(payload) {
  // The scheduler hands over every absolute candidate root, so the config root
  // and the data root of one installation can both arrive here. Keep one
  // installation per data namespace, preferring the candidate that also names
  // the enclosing config root.
  ...
  for (const roots of byData.values()) {
    installations.push({
      ...
      evidence: [
        {
          description: 'Codex sessions namespace exists',
...
function collectEnvelope(request, startedAt) {
...
async function main(request) {
  ...
  if (request.action === 'collect' || !request.action) {
    if (request.capability === 'sessions' || request.capability === 'usage')
      return collectEnvelope(request, startedAt)
